...
const auth = {
  getAuth: function () {
    let url =
      ...
      "?response_type=token" +
      "&client_id=" +
      encodeURIComponent(authCreds.client_id) +
      "&scope=" +
      encodeURIComponent(authCreds.scope) +
      "&redirect_uri=" +
      encodeURIComponent(authCreds.redirect_uri) +
      "&state=" +
      encodeURIComponent(authCreds.state);
  ...
  getHashCode: function () {
    ...
    if (token) {
      document.cookie =
        "spotiToken=" + token + ";max-age=3600;samesite=lax;Secure";
  ...
  setUserID: async function (token) {
    await axios({
      ...
      if (res) {
        document.cookie =
          "spotiUID=" + res.data.id + ";max-age=3600;samesite=lax;Secure";
        if (res.data.display_name) {
          document.cookie =
            "spotiUN=" +
            res.data.display_name +
            ";max-age=3600;samesite=lax;Secure";
        }
        window.location.hash = "";
      }
    });
  ...
  checkCookie: function () {
    if (
      document.cookie
        .split(";")
        .some((item) => item.trim().startsWith("spotiToken="))
    ) {
